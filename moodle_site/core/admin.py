...
from .models import (
    ...
    File, FileShare, Message, UserMessage
)
...
class FileAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'description', 'file', 'original_filename', 'extension')
        }),
        ('File Data', {
            'fields': ('uploaded_by', 'date_uploaded', 'checksum_hash')
        }),
        ('VirusTotal Scan', {
    ...
    def save_model(self, request, obj, form, change):
        if not change:  # Only for new files
            obj.uploaded_by = request.user
            if 'file' in form.files:
                obj.size = form.files['file'].size
